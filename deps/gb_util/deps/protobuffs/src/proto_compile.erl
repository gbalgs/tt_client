...
-spec scan_file(ProtoFile :: string() | atom(), Options :: list()) ->
		       ok | {error, _}.
scan_file(ProtoFile,_Options) when is_list(ProtoFile) ->
    {ok,String} = parse_file(ProtoFile),
    parse_and_gen_file(String);
scan_file(ProtoFile,_Options) when is_atom(ProtoFile) ->
    {ok,String} = parse_file(atom_to_list(ProtoFile) ++ ".proto"),
    parse_and_gen_file(String).

scan_dir(SrcDir, ErlOutDir,HrlOutDir) ->
	MsgList = filelib:fold_files(SrcDir, ".+\.proto", false, fun(FileName, Acc) ->
														   ...
														   []),
	parse_and_gen_file(MsgList, ErlOutDir, HrlOutDir).
...
parse_and_gen_file(TokenList) ->
	parse_and_gen_file(TokenList, ".", ".").
parse_and_gen_file(TokenList, ErlOutDir,HrlOutDir) ->
    %{ok, TokenList} = parse_string(String),
	check_duplicate(TokenList),
	Tokens1 = sort(TokenList),
    Hrl = gen_hrl(Tokens1),
	Tokens = type_trans(Tokens1),
%% 	CppH = gen_cpp_h(Tokens),
	...
	Hrl1 = ["-define("++string:to_upper(Name)++","++integer_to_list(MsgID)++").\n"||{message, Name, MsgID, _Route, _Fields} <- Tokens],
	DefineType = "-define(int8, type:int8()).\n-define(int16, type:int16()).\n-define(int32, type:int32()).\n-define(int64, type:int64()).\n-define(uint8, type:int8()).\n-define(uint16, type:int16()).\n-define(uint32, type:int32()).\n-define(uint64, type:int64()).\n-define(string, type:str()).\n\n",
    file:write_file(filename:join([HrlOutDir, "all_proto.hrl"]),["-include(\"define_proto.hrl\").\n"|[DefineType|Hrl]], [{encoding,utf8}]),
    file:write_file(filename:join([HrlOutDir, "define_proto.hrl"]),Hrl1, [{encoding,utf8}]),
	file:write_file(filename:join([ErlOutDir, "proto_route.erl"]),RouteFile, [{encoding,utf8}]),
    file:write_file(filename:join([ErlOutDir, "proto_struct.erl"]),[all_proto_erl_header(), EncodeDef, DecodeDef, GetID], [{encoding,utf8}]).


type_trans(Tokens)->
	lists:map(fun({message, Name, _MsgID, _Route, Fields})->
		NewFields =
			lists:map(fun({_Number, RepeatedType, Type, _FieldName, _DefaultValue})->
				NewType =
					case Type of
						"uint8"->
							"int8";
				...
				{_Number, RepeatedType, NewType, _FieldName, _DefaultValue}
			end,Fields),
		{message, Name, _MsgID, _Route, NewFields}
	end,Tokens).
										  ...
										  _->
											  case MsgStr of
												  "cs_" ++ MsgStr2 ->
													  ...
													  Case = ["route(?",string:to_upper(MsgStr),") ->\n\t{",Server, ",", HandleModule,"};\n"],
													  [Case | Acc];								
												  _ ->
													  Acc
											  end
...
-define(TypeList, ["int8","int16","int32","int64","uint8","uint16","uint32","uint64","bool","string","any"]).
...
gen_encode_def(String) ->
    [gen_encode_def2(Name, Fields) || {message, Name, _MsgID, _Route, Fields} <- String].
gen_encode_def2(Name, Fields) ->
    Head = "encode_def(?"++string:to_upper(Name)++", R)->\n\t",
...
gen_decode_def(String) ->
    [gen_decode_def2(Name, Fields) || {message, Name, _MsgID, _Route, Fields} <- String].
gen_decode_def2(Name, Fields) ->
    Head = "decode_def(?"++string:to_upper(Name)++")->\n\t[",
    Body = string:join([Name| 
        [gen_decode_def3(RepeatedType, Type)||{_Number, RepeatedType, Type, _FieldName, _DefaultValue}<-Fields]
        ],
        ","
        ),
    Tail = "];\n",
...
gen_decode_def3(repeated, Type) ->
    "{list, "++ encode_type(Type) ++"}";
gen_decode_def3(_RepeatedType, Type) ->
    encode_type(Type).
...
filter_decode_extensions_clause(Msgs,[{MsgName,_,Extends}|Tail],Clause,Acc) ->
    ...
    Cons = lists:foldl(
	     fun({FNum, FName, Type, Opts, _Def}, _Acc) ->
...
collect_full_messages([{extend, Name, ExtendedFields} | Tail], Collected) ->
    ...
    #collected{msg = CollectedMsg, extensions = _Extended} = Collected,
